class GameScene extends Phaser.Scene {
    ...
    createBackground() {
        try {
            ...
            const maxWidth = 800; // Maximum width for the image, matching our new chibi size
            const scale = 1; // Since we designed it at this size, no scaling needed
            this.chibiImage.setScale(scale);
            
            // Set semi-transparent to show it's covered
            ...
            this.revealMask.fillRect(0, 0, 1920, 1080);
            ...
            console.log("Background created with chibi image:", 
                        this.chibiImage.width, "x", this.chibiImage.height,
    ...
    createIceBlocks() {
        ...
        console.log(`Created ${this.iceBlocks.length} ice blocks`);
    }
    ...
    destroyIceBlock(block) {
        ...
        if ((previousPercentage < 20 && this.revealPercentage >= 20) ||
            ...
            // Gradually increase the opacity of the image as more is revealed
            const newAlpha = 0.5 + (this.revealPercentage / 100) * 0.5;
            this.chibiImage.setAlpha(newAlpha);
    ...
    checkLevelCompletion() {
        ...
        if (this.revealPercentage >= this.targetPercentage) {
            ...
            // After a delay, show restart button or next level option
            this.time.delayedCall(3000, () => {
    ...
    createBombSelector() {
        ...
        const buttonY = 920;
        const spacing = 100; // Increased spacing for larger resolution
        
        // Blast bomb button
        this.blastButton = this.add.image(this.cameras.main.width - 600, buttonY, 'blast_bomb');
        this.blastButton.setScale(2); // Larger buttons for higher resolution
    ...
    handleStickyBomb(x, y, block) {
        // Sticky bomb adheres to the first block it hits and explodes after a delay
        
        // Visual effect for sticky attachment
        const stickEffect = this.add.particles('sticky_particle');
        const emitter = stickEffect.createEmitter({
            speed: { min: 10, max: 30 },
        ...
        // Position the bomb to stick to the block
        this.bomb.setStatic(true);
        this.bomb.setPosition(block.x, block.y);
        
        // Emit sticky particles
        emitter.start();
        
        // Create a countdown effect
        const countdownText = this.add.text(block.x, block.y - 30, '3', {
            font: '24px Arial',
            fill: '#ffffff',
            stroke: '#000000',
            strokeThickness: 3
                    ...
                    onComplete: () => {
                        ...
                        this.tweens.add({
                            ...
                            onComplete: () => {
                                ...
                                // Destroy all blocks in a larger radius since it's a focused explosion
        ...
        // Pulse animation for the bomb while waiting
        this.tweens.add({
            targets: this.bomb,
            scale: 1.2,
            duration: 500,
